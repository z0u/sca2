...
- A **tick** notes every memo hit whose result predates an upgrade, and logs one warning naming what moved (`jax 0.10.1 → 0.11.0`) and how many hits it covers. Said once per process — a watching driver ticks every few seconds and the answer doesn't change.
