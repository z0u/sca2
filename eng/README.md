...
- [Reproducible GPU runs](./determinism.md) — why the same seed gave three different models, what the `XLA_FLAGS` we set buy and cost, why the flags live in the environment rather than in Python, why a JAX bump moves the digest without moving the memo key, and why "just tolerate small differences" doesn't survive contact with a content-addressed DAG.
