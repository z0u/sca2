...
def make_dopesheet(peak_lr: float) -> str:
    """Ex-2.9.1's dopesheet (timed anneal) with a parameterized LR plateau, as in ex-2.9.3."""
    rows = [
...
def score_interventions(params: Params) -> dict:
    ...
    out = {}
    for name, bias in (("zero", 0.0), ("redirect", -ALPHA_RD)):
...
def train_one(seed: int, peak_lr: float, ctrl: bool, tau_scale: float, eta_scale: float, w_fb: float) -> dict:
    ...
    """
    sheet = Dopesheet.from_csv(io.StringIO(make_dopesheet(peak_lr)))
    ...
    buf = io.BytesIO()
    mse_rd, z_rd = eval_model(edit_axis0(params, bias=-ALPHA_RD), x_train)
    np.savez_compressed(
        buf,
        rgb=GRID_RGB,
        sim3=SIM3,
