...
def _(sweep_cell):
    ...
    _alpha_rd = [r for r in _hot if r["interventions"]["redirect"]["red_pure"] < 0.05 and classify(r) == "clean"]
    _hot_ok = np.array([r["interventions"]["redirect"]["score"] for r in _hot if r not in _alpha_rd])
    mo.md(
        ...
        f"anchoring failure at all — the run anchored cleanly, but the redirect's fixed α_rd = 1 bias "
        ...
        f"That is ex-2.9.2's α_rd-calibration caveat recurring in 1 run of 256; excluding it, the hot "
        f"cell's floor is {_hot_ok.min():.2f}. α_rd should be calibrated per model, not fixed."
    )
    return
...
def _(arm, sweep_cell):
    ...
    - **Calibrate the redirect's α_rd** against the model's pre-norm activation
      scale; a fixed α_rd = 1 silently no-ops on ~1 run in 250.
