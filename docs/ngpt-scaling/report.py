...
def _():
    ...
    The model is a simplified version of *nGPT*. The idea behind nGPT is to keep the running state of the model (the *residual stream*, the vector that each layer reads from and writes back to) on the surface of a hypersphere, by normalizing it after every step. We keep that residual update, `h ← Norm(h + α·(Norm(sub(h)) − h))`, which moves the state `h` a fraction `α` of the way toward the normalized output of a sub-module and then renormalizes. We simplify two pieces of it. Each sub-module gets a single learned *gain* (one number that scales its output) in place of the nGPT per-channel *eigen learning rates*, and the residual step `α` is fixed at 1/n_layer instead of being learned, which is about where the learned version settled anyway.

    For the claim that SCA (the concept-anchoring method this project studies) carries over to language models, this pared-down architecture needs to stay well-behaved as it scales. So the [experiment](./experiment.py) trains it across a grid: three widths (how many numbers are in that state vector) crossed with three depths (how many layers), {32, 64, 128} × {4, 8, 12}, with everything else held fixed: batch size 16, peak learning rate 10⁻², 100 epochs, and *Pride and Prejudice* for the training text. Two outcomes would concern us. One is a **depth penalty**, where adding layers at a fixed width makes the model worse. The other is an instability that shows up only at large width, where a run spikes or fails to train. We hope to see neither.
    """)
    return
...
def _(loaded):
    ...
    mo.md(
        f"""
    **The architecture scales cleanly.** We score each run by its converged loss: the average model error at predicting the next character once training has settled, measured in *nats per character* (natural-log units, where lower is better). That loss never rises as we add layers. At each width, the three depths land within {depth_spread:.02f} nats/char of one another, well inside the ±0.08 of noise we see between epochs, so the depth axis is flat. Width behaves the way added capacity should: loss falls monotonically from {plateau(curves, 32, 4):.2f} at width 32 to {plateau(curves, 128, 12):.2f} at width 128 with 12 layers. No condition spikes or stalls, and the same learning rate (10⁻²) works everywhere. So fixing the residual step to a scalar constant seems to be enough.
    """
    )
    return (curves,)
...
def _():
    ...
    The same runs, now shown across training: one panel per width, one line per depth, loss on the vertical axis and epoch on the horizontal. Every run descends through the learning-rate warmup (the opening stretch of training, where the step size ramps up from small) and settles onto a plateau. Within a panel, the depth lines sit on top of one another rather than fanning apart, and the wider panels settle lower.
    """)
    return
