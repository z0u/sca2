...
- [2.2.7. A pilot of the syntax rows](./m2/ex-2.2.7/report.py)
    ...
    A pilot, no gates, nine seeds per arm, on the anchor axis that every anchored model carries on the embedding rows of the op words and `=`. Part A strips the component from ex-2.2.3's stored checkpoints on the input side or the output side of the tied table: the output side carries the `=` prediction after a red operand, which is the tied-readout mechanism as hypothesised, and the input side reads as the blocks having learned where the rows sit. Part B retrains the recipe three ways, each removing one candidate mechanism. Anchoring the blocks only does not clean the rows and makes the projection less complete, since half of a red operand's redness then enters the stream off the axis; untying the readout moves the component onto the readout table; holding the rows at zero after every step cleans them but selects rows by token class, so it is the in-grammar ceiling rather than a transferable fix. Untying and the row constraint both cost nothing visible and lower the full-position projection's non-red cost. The pilot proposes the untied readout for the handover, and flags a selectivity tail under the whole-line labeller.
