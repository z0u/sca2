...
def _():
    ...
    Fitted probes remove that asymmetry. A probe fit at a site reads whatever color representation the model keeps there, whether that is the anchored one or the model's own. So if the tilt comes from the probe set rather than the model, it should show up in every condition, including the un-anchored control, and it should stay under the information ceiling set by the pairing, which we can compute in advance (H1). [Ex-2.1.5](../ex-2.1.5/report.py) ran the same kind of scan on an un-anchored two-form model, and there the op2 probes read zero at op1 sites. That experiment sampled its lines over distinct operand pairs, so the operands were all but uncorrelated and the ceiling was effectively zero. Comparing the two probe sets is what makes the case: the tilt follows the pairing, and the model has no say at these sites.
    ...
    **Noise floor.** The primary condition has nine seeds and the others three. The pooled between-seed standard deviation of per-site R², computed per condition before any comparison is read, sets the resolution: a difference smaller than twice that floor is reported as not resolved.
    ...
    - **H1.** At the two sites that precede op2 — the op1 and `+` positions, every slice — the seed-mean strict R² of the op2-target probe stays below 0.12 in every condition and every channel. The ceiling is not a property of any model: the state at these sites is a function of the tokens before op2, so the best any probe can do is E[op2 | op1], and the closure rule caps that at R² = 0.25 / (35/12) ≈ 0.086 per channel; the gate adds margin for estimation noise. A reading at or above 0.12 falsifies the bookkeeping account — it would mean the probe found more op2 information before op2 than the pairing supplies, i.e. a leak in the protocol or a misread of the probe set. We also expect, without gating it, that the readings are similar across conditions (the control included) and across channels, since the pairing is channel-symmetric; how close they come to the ceiling depends on each model's embedding geometry and is reported, not scored.
    - **H2.** Anchoring does not cost linear color decodability where the control has it. For each target, the comparison site is chosen by the control alone: the (slice, position) of the control's best seed-mean strict R² (mean over channels), among targets whose control best reaches R² ≥ 0.5. At each such site, the primary's seed-mean per-channel R² comes within 0.10 of the control's (one-sided: the primary may exceed it). Partial: exactly one channel of one target misses, by no more than 0.2. The intermediate conditions are reported at the same sites without a gate.
    """)
    return
...
def _():
    ...
    /// admonition | TODO
    Per-channel strict R² maps for the op2 target, one panel per condition: slices upward from the embedding, positions across, one step-line per channel (the `probe_trace_grid` layout of ex-2.1.5). A horizontal rule at the 0.086 ceiling over the op1 and `+` positions. Expected: every condition, control included, shows a small bump at those positions at or below the ceiling, and the genuine response from the op2 position onward. Contrary: any pre-op2 reading clearing 0.12, or the control showing nothing pre-op2 while anchored conditions show the ceiling (which would mean the tilt is *not* model-independent).
