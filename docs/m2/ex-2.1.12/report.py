...
def _(r2):
    @themed(
        ...
        """,
        caption=r"""
            **Per-channel strict probe R², per condition and target.** Rows are the four conditions of the D2.1 progression; columns the probe targets. Within a block, slices run upward from the embedding, the x axis crosses the six token positions, and each RGB channel draws as its own step-line, with the grey area their mean and hairlines the seed envelope. Negative scores clip to the floor: a probe that does worse than predicting the mean has failed, and how much worse is not a finer grade of failure — the H1 prose carries the raw values. One scale, 0 to 1, serves every panel.
        """,
    )
