...
def _():
    ...
    **H4.** The R² of the embedding-to-RGB probe and the low-dimensionality of the embedding table (top-3 explained variance) rise with grid size, and a PCA of the embeddings shows the color cube where, and only where, held-out performance is good.
...
def _(evals, metrics):
    ...
    mo.md(f"""
        **Headline numbers.** Mean held-out accuracy by grid: {", ".join(f"`{g}` **{v:.2f}**" for g, v in _hold.items())}. On every grid but the smallest, that is far above what a model knowing only the neighborhood of the answer would score ({", ".join(f"`{g}` {v:.2f}" for g, v in _null.items())}), so the model does infer the geometry (except `v27`, which is discussed below). Misses are interesting: at `v4096` they land a mean distance of just {_mean("v4096", "named_holdout", "guess_dist"):.3f} from the true mix (chance is {_mean("v4096", "named_holdout", "chance_dist"):.2f}). The sections below build that picture up piece by piece, starting with the pairs the corpus sampler actually produced.
    """)
    return
...
def _(arrays, evals, geom, metrics):
    ...
    mo.md(rf"""
    Every grid learns its seen pairs almost perfectly, apart from `v4096`, which reaches {_acc("v4096", "named_seen"):.2f}. That is the one grid where even the training pairs are hard to memorize, since its corpus visits 99k distinct pairs roughly once each. Held-out accuracy then follows a non-monotonic path: {_acc("v27", "named_holdout"):.2f} at `v27` (that's {round(_acc("v27", "named_holdout") * 10)} of its ten held-out pairs), {_acc("v64", "named_holdout"):.2f} at `v64`, {_acc("v216", "named_holdout"):.2f} at `v216`, near enough to solved, and back down to {_acc("v4096", "named_holdout"):.2f} at `v4096`.

    Only three of those four numbers mean much. On a 27-color grid the true mix has just {_shell27:.1f} one-step neighbors on average, so a model that has located the neighborhood of the answer and picks within it scores {_null27:.2f} without being able to name anything. The `v27` score of {_acc("v27", "named_holdout"):.2f} sits {_z27:.1f} std above that, over ten distinct pairs, which is not a difference worth reading. The denser grids clear the same bar easily: {_z64:.0f} std at `v64`, {_z216:.0f} at `v216` and {_z4096:.0f} at `v4096`.

    So the `v27` score is not evidence on its own. It is tempting to read it as a shift from the base language, where `named_holdout` sat at zero through every intervention ex-2.1.2 tried, but two of ten pairs on a grid this coarse cannot carry that. The case that the geometry is inferable from names rests on the denser grids here, and on the embedding probes further down, which reach R² ≈ {_r2_27:.2f} even at `v27`.

    The `v4096` misses are not scattered, either. Pooled over seeds, {_m4096[1]} of {_m4096[0]} held-out misses differ from the true mix in a single RGB channel, and {_m4096[2]} of those are off by one grid level (`v64`: {_m64[2]}/{_m64[0]}; `v216`: {_m216[2]}/{_m216[0]}). We checked whether these misses are more likely on rounding cases, meaning channels where the operand sum is odd so the mean has to round, and they don't: about half of the one-channel misses are rounding cases, which matches chance.
    """)
    return
...
def _(arrays, evals, metrics):
    ...
    mo.md(rf"""
    Guesses are better than chance everywhere, and comfortably inside the prompt-blind constant, which is stricter: mixes are midpoints, so they cluster centrally, and always answering the middle name scores {bl.blind_stats(dist_matrix("v27", evals["v27"]["open"]), blind_for("v27", evals))["dist"]:.2f} on `v27` open pairs where chance is {_mean("v27", "open", "chance_dist"):.2f}. That much supports the picture in H3.

    {_table}

    The floor is a harder reference than it looks, though, because it asks the model to break a tie it may have no way to break. An open mix falls between grid points, so two names bracket it, often at the same distance. A model that has located the answer but picks between those two at random is the "2 nearest" column, and every grid sits behind it: `v27` guesses {_mean("v27", "open", "guess_dist"):.3f} against {bl.k_nearest_stats(dist_matrix("v27", evals["v27"]["open"]), 2)["dist"]:.3f} for that baseline. On the nearest-name rate the gap is wider, {_mean("v27", "open", "nearest_acc"):.2f} against {bl.k_nearest_stats(dist_matrix("v27", evals["v27"]["open"]), 2)["nearest"]:.2f}. So the guesses are near the true mix, but not as near as a model with the same positional knowledge and a coin could get.
...
def _(geom, metrics):
    ...
    Probe R² quantifies how well it fit: {geom("v27", "r2"):.2f}, {geom("v64", "r2"):.2f}, {geom("v216", "r2"):.2f}, {geom("v4096", "r2"):.2f} across the four grids.[^folds] From `v64` up, RGB is close to a linear function of the embedding.
    ...
    At `v27` the answer is a coin flip. `v4096` makes the same point from the other end: its R² of {geom("v4096", "r2"):.2f} is respectable, but sixteen levels per channel pack the names so tightly that a decoded token is almost never nearest its own. So the probe finds the cube, but it does not decode individual tokens well enough to name them at any of these grid sizes.

    H4 expected the embedding table itself to turn low-dimensional, and the PCA says it doesn't: even the best three directions hold only {_evr["v27"]:.0%} of the variance at `v27`, and that falls to {_evr["v4096"]:.0%} at `v4096`. The cube is in there as a subspace the probe can read, but most of the embedding variance is doing something else.

    [^folds]: Probes fit per-color with leave-one-out, which matters most at the
    small grids. A single half/half split leaves the `v27` probe fitting a 64→3 map from 13 points, and scores it {_cell_mean("v27", "emb_r2"):.2f} against {geom("v27", "r2"):.2f} here; `v4096` is unmoved either way ({_cell_mean("v4096", "emb_r2"):.2f} against {geom("v4096", "r2"):.2f}).
    """)
    return
...
def _(arrays, evals):
    ...
    Several of the `v27` rows return one of the operands, may or may not be an echo. Returning an operand and returning a neighbor are close to the same thing here. {_k} of {_n} guesses are operands, against {_null:.0%} for a guess drawn uniformly from the neighbors.
    """)
    return
