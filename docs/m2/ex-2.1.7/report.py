...
def _():
    ...
    threshold, flat across a tenfold range of the anchor weight. That result
    left two candidate mechanisms confounded.

    First, nothing repelled the rest of the cube. M1 never ran its anchor
    bare: an *anti-subspace* term, at a few percent of the anchor weight,
    constrained the mean-square alignment of every point — exactly the
    quantity that ran away in ex-2.1.6.
    ...
    the label is an unobservable coin flip, so only the expected pull is
    visible to the optimizer, and the expected pull is nearly
    color-independent — a drift toward the anchor by construction.

    This experiment separates the two with a 2×2 factorial at ex-2.1.6's
    ...
    restores the margin, the missing-repulsion account wins; if the op1-only
    factor does, the blind-span account wins; if only the combination does,
    they compound. Two arms ride along: a schedule-timing variant of the
    ...
    /// details | Notation
    Ex-2.1.6's notation carries over: $\hat v_{\text{red}}$ is the anchor
...
def _():
    ...
    `redness(op1)⁸ × 0.08`, the exhaustive 27-partner alignment probe set,
    and the same measurements — the alignment map over layers × positions,
    the label-affinity-weighted margin, off-axis leakage probes, and the
    margin trajectory every 50 steps. The [ex-2.1.6
    report](../ex-2.1.6/report.py) documents each; only what changes is
    described below.

    One measurement is promoted rather than new. Ex-2.1.6's exploratory
    geometry pass — the centroid of the 216 op1 states, its alignment with
    the anchor, and the cloud's extent, per layer — is preregistered here,
    because H4 scores part of it. The mean alignment over all colors,
    ...
    All four cells run at $\lambda = 0.1$, ex-2.1.6's scoring rung, chosen
    there because every rung was task-clean and 0.1 sat in the middle of the
    swept range. The control ($\lambda = 0$) and the `span-bare` cell
    reproduce ex-2.1.6's control and its λ=0.1 condition, so every
    ...
    **Factor one: the anti-subspace term.** The `anti` cells add M1's
    ...
    the mean-square alignment of everything, so alignment becomes something
    the labeled pull has to buy against a headwind, which is the selectivity
    pressure ex-2.1.6 lacked. Note what it does not do: it never asks any
    particular point to leave the axis, only that the cloud as a whole not
    sit on it.

    Capacity is worth a sentence. In M1's 4- and 5-dimensional bottlenecks,
    keeping the cloud off one axis surrendered a fifth or a quarter of the
    space, and the harder M1 conditions (ex-2.9.1) needed the dimension
    cleared outright. Our stream is 64-dimensional: an isotropic cloud has a
    mean $\cos^2$ of 1/64 per slice, and the task's three color dimensions
    fit in the remaining 63 with room to spare. So the term should be cheap
    here — the live question is whether, at 3% of the anchor weight, it is
    strong enough to matter, not whether the task can afford it.
    ...
    **Factor two: the pull span.** The `span` cells pull the four prompt
    ...
    no repulsive term at all. The span pull remains the shape natural
    language forces (a document label marks no position as the relevant
    one), so if op1-only wins, the lesson is about what sequence-level
    labeling costs, not a design we can keep for M3.
    """)
    return
...
def _():
    ...
    The anchor schedule is ex-2.1.6's, unchanged: ramp over the 10-epoch LR
    warmup, hold at $\lambda$, anneal from epoch 90 to a floor of
    $0.1\lambda$ at 100.
    ...
    opened at 2.5× the anchor's peak weight — at full strength from step 0,
    before the anchor had ramped in — and annealed to 3% of the anchor by
    the halfway point, where it held. Mapped onto our frame:
    ...
    How robust that timing is, nobody has measured — M1's own schedules
    varied by experiment (ex-2.4.1, a gentler condition that confined
    vibrant colors to a two-axis subspace rather than clearing an axis, ran
    its terms *up* over training instead). So one arm probes it:
    `span-anti-late` stretches the anneal's endpoint from epoch 50 to 90,
    ...
    one rung above ex-2.1.6's swept maximum, with $\mu$ scaled in
    ...
    available to spend. It probes the recipe's headroom, not the bare
    anchor's: both terms scale together.
    """)
    return
...
def _():
    ...
    def _row(c: dict) -> str:
        ...
        span = "op1 + op2 =" if c["span"] == ex.SPAN_FULL else "op1"
        role = {
            "lam0": "control",
            "span-bare": "ex-2.1.6's λ=0.1 cell, re-run",
        ...
        return (
            ...
            f"<td><code>{span}</code></td><td class='num'>{anti}</td><td>{role}</td></tr>"
        )
...
def _():
    ...
    @themed(
        ...
        caption=f"""
            ...
            {ex.ANTI_ANNEAL_END:g} (M1/ex-2.9.1's keyframes, mapped by fraction of
...
def _():
    ...
    reported on the `span-anti` cell (seed-averaged), the condition
    ex-2.1.6's discussion named as the next experiment; the other factorial
    ...
    **H1.** The added term and the narrower pull are as free as the bare
    ...
    within 0.02 (absolute) of the in-experiment control's seed mean. The
    ceiling arm is reported in the same table but scored only
    ...
    **H2.** With the missing ingredient restored, the concept lands
    ...
    graded — over the 216 colors, the seed-mean layer-mean alignment at op1
    clears Spearman $\rho \ge 0.8$ against `redness` or Pearson
    $R^2 \ge 0.8$ against `sim¹·⁵` (both tracks, thresholds, and step
    ceilings as justified at length in ex-2.1.6); (c) the control shows
    $|m_{\text{op1}}| \le 0.1$. Partial: some cell clears 0.35 —
    above ex-2.1.6's 0.27 by more than the seed-mean noise — without
    reaching 0.5.

    **H3.** The margin's failure in ex-2.1.6 was mostly the missing
    repulsion, not the blind span. Scored on the factorial's main effects
    on $m_{\text{op1}}$ (each factor's mean over its two cells, differenced):
    the anti-subspace main effect is at least 0.1 — about 4× the three-seed
    noise floor — and at least as large as the op1-only main effect.
    Partial: the op1-only effect clears 0.1 instead, which reads as the
    blind-span account winning; or neither main effect clears 0.1 but the
    `op1-anti` cell alone does (an interaction: each mechanism blocks the
    margin on its own).
    ...
    **H4.** The dynamics behave as the term dictates. (a) Containment: in
    ...
    over all colors at op1 satisfies $\bar\alpha \le 0.25$ — against 0.53
    for the bare term in ex-2.1.6, and ≈ 1/8 for an isotropic cloud.
    (b) Retention: ex-2.1.6's trajectory gate, unchanged — for every
    anchored run whose running maximum of $m_{\text{op1}}$ reaches 0.2, the
    final value is at least 0.8× that maximum. Ex-2.1.6's margin peaked
    near epoch 10 and slid back a quarter under steady pressure; if the
    slide was the rest of the cube catching up to a selective early
    response, repulsion should hold the peak. Partial: (a) holds and (b)
...
def _():
    ...
    Ex-2.1.6's H1 table, one row per condition plus the ex-2.1.3 `v216`
    reference and the published ex-2.1.6 λ=0.1 row: seen and held-out exact
    match, held-out NLL, `open` guess distance, the signed gap to control,
    and the gate verdict. Expected: every λ=0.1 row within 0.02 of control,
    replicating H1; the ceiling arm is where a cost would first appear, and
    a clean ceiling arm would say the task tolerates the recipe at 10× the
    scoring weight. Contrary: a cost in the anti cells but not `span-bare`
    would mean the repulsive term, not the anchor, is what the task feels.
    ///
    """)
    return
...
def _():
    ...
    seed-mean bar), with the 0.5 gate, the 0.35 partial line, and
    ex-2.1.6's published 0.27 as a reference line. Expected: `span-anti` at
    or above the gate; `span-bare` reproducing 0.27. Contrary: all four
    cells in a band around 0.27 — neither mechanism was the bottleneck.
    ///
    ...
    *red*, and the flattened-affinity follow-up already queued in
...
def _():
    ...
    op1-only effect. Each of the four orderings has a named reading in H3;
    state which one obtained and carry it to the discussion.
    ///
    """)
    return
...
def _():
    ...
    with the 0.25 containment gate and ex-2.1.6's bare-term endpoint (0.53)
    marked. Right: $m_{\text{op1}}$ with its running maximum, the 0.8×
    retention band, and both weight schedules behind it. Expected: anti
    ...
    the anneal ends — exactly what the timing arm is positioned to confirm,
    since it keeps the weight up through epoch 90.
    ///

    /// admonition | TODO: per-layer and geometry read
    The margin by layer × position for `span-anti` beside `span-bare`
    (ex-2.1.6's figure, one new column), and the preregistered geometry:
    centroid·anchor and extent per layer. Expected: centroid alignment near
    the control's (0.02 in ex-2.1.6) rather than the bare term's 0.89 at
...
def _():
    ...
    trajectories. Same-within-noise says the M1 recipe is robust to a 2×
    stretch of its anneal and the timing question can rest; a gap in either
    direction says schedule timing is load-bearing and sizes the dedicated
    sweep. Report descriptively; no gate.
    ///
    ...
    weight budget for D2.2's operation anchors; if it is still free, note
    that the ceiling remains unfound and leave chasing it to a dedicated
    sweep.
    ///
    """)
    return
...
def _():
    ...
    next — the flattened label affinity if selectivity appeared, the
