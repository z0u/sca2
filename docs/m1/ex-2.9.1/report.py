...
def _():
    ...
    main reconstruction objective, that shapes the geometry of the latent space without changing what the model is fundamentally trying to do.
    """)
    return
...
def _(loaded):
    ...
    mo.md(f"""
    {len(metrics)} runs completed. Scores range {min(scores):.2f}–{max(scores):.2f} (median {np.median(scores):.2f}): anchoring quality depends on the seed, as it did in the original. The best run is seed {best["seed"]} with score **{best["score"]:.3f}**, in line with the best of 0.985 over 60 seeds in the original.
    """)
...
def _(best_eval):
    ...
    mo.md(f"""
    Baseline reconstruction is near-perfect (mean MSE {_mse_base.mean():.2e}). After ablation, red-like colors (similarity³ > 0.5) have a mean error of {_mse_abl[_reds].mean():.3f}, while colors unlike red (similarity³ < 0.01, {int(_others.sum())} of {len(_sim)} grid points) sit at {_mse_abl[_others].mean():.2e}: deleting red barely affects them.
    """)
    return
...
def _():
    ...
    Next: the M2 experiments proper, anchoring concepts in the residual stream of a small transformer on the color-mixing task (see the [README](../../README.md)).
    """)
    return
