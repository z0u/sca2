...
**2026-09-02, housekeeping** — still unstarted, and still the gate on the op experiments: `src/sca/data/colors.py` has one op spelled `+`, and `Example` carries `prompt`/`answer` with no `op` field. Worth knowing why D2.2 has moved without it, so this doesn't read as overtaken: D2.2's first experiment, [ex-2.2.1](/docs/m2/ex-2.2.1/report.py), suppresses *red* on the D2.1 checkpoints, so it anchors a color rather than an operation and needs only the existing one-op language. Everything after it in the plan does need the table.
