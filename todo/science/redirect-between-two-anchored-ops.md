...
Anchor two operations, then intervene by redirecting one's component to the other: a model asked to `add` computes `multiply`.
...
The D2.2 [design](/docs/m2/d2.2/design.md)'s fallback discussion adds a third: the redirect target is a state training visits in the ordinary course of the task, so the intervened response is trained without rehearsing any operator ([rehearsed fallback](rehearsal-fallback-as-auditing-probe.md)) and without the antipode's train/eval mismatch — the designed-response claim with the fewest caveats available to us.
