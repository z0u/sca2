...
**2026-08-26, housekeeping** — The conflict estimate above is wrong, and landing this is cheaper than it says. `git merge-tree --write-tree origin/main <branch>` returns a tree with no conflicts today: the branch is 64 commits behind and 22 ahead, and `.agents/skills/style-py/SKILL.md` is the single file both sides touched, which auto-merges. `style-fig` is untouched on `main` since the 08-15 merge base, so the branch's rewrite of it applies clean. That leaves the `./go deadcode` finding on the three preregistration constants as the only known obstacle, and it is on a `continue-on-error` step.

Still unmoved since 08-19 — the branch head is `de201e7`, and PR #99's description is unchanged. Worth knowing that nothing else has landed in its way either: no experiment work has reached `main` since ex-2.1.11 on 08-12, so everything merged in the fortnight since is tooling and housekeeping. That cuts both ways — the ex-2.1.12 number is in no danger of being claimed by an unrelated session this week, and D2.1's last unpublished result is the thing D2.2 is waiting behind.
