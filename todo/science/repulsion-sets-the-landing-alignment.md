...
[Ex-2.2.1](/docs/m2/ex-2.2.1/report.py) ran M1's shaped suppression as an arm: remove a fraction $h(\alpha)$ of the e₁ component, zero below a threshold $a = 0.5$ and rising linearly to all of it at $\alpha = 1$, then re-normalize. The re-normalization is what makes the operator a rotation, and it also decides where the state lands, which the suppression does not control. Three things followed, all in the report's operator figure and E4.
...
- The blocks then bring the red operand partway back. At slices 1–3 it arrives at a mean alignment of about 0.33 under the shaped suppression (0.52 clean), and about half of those states sit above the threshold and are written again.
- The shaped suppression removes about half of the red response (red accuracy about 0.6 against 0.09 under the projection), at no non-red cost. What it decodes to is still a color: off-vocabulary mass on red lines is 0.002, 60% of decodes are the true answer, and 29% are a one-step neighbor.
...
- For *removal*, the shaped suppression is the stronger operator at a given threshold: repulsion with a ceiling at $a = 0.5$ would leave pure red at 0.5, above where the suppression puts it, so it would bite less. Ex-2.2.1's data also says the worry that a state pushed past the threshold behaves at random is not what happens in this model: under the projection, the far case, 98.5% of the wrong decodes are what the line would mix to with a less red operand, and off-vocabulary mass stays small.
