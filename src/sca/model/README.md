...
In a normal transformer, layer norm rescales activations in a way that distorts their direction (see _Rotary positional encodings_, below). nGPT instead L2-normalizes everything — token embeddings, the residual stream, the attention queries and keys, and the weight matrices — so a vector's length is fixed at 1 and only its angle can vary. The paper reports faster convergence and better length generalization; for us, the draw is that the geometry now matches the "direction-as-meaning" intuition we rely on.
...
- The sequence length is not an inherent part of the model. GPT-2 used learned positional encodings; this code uses RoPE, which makes it possible to extend the context length after training[^length].
...
Rotary positional encodings (RoPE) work differently from earlier schemes. GPT-2 (which nanoGPT replicates) uses learned embeddings, and the original Attention is All You Need paper used sinusoidal patterns; in both cases they were applied to the _input_ token embeddings. RoPE also uses sinusoidal patterns, but in attention space ($Q$, $K$).

RoPE applies sequence position _as rotation_ of the queries and keys, rather than adding it to the token embeddings. So the attention mechanism can learn positional relationships in a way that suits "direction-as-meaning" embeddings, and it generalizes to longer sequences than were seen during training. GPT-2 limited that generalization by applying layer norm in each `Block`: layer norm is non-geometric, so it distorts the directional interpretation of the vectors. In nGPT there is no layer norm to fight against — rotation and normalization are both geometric operations on the sphere, so they compose cleanly.

The queries and keys are L2-normalized (per head) immediately after RoPE. Normalization and rotation commute — rotating a unit vector leaves it a unit vector — so the order doesn't matter mathematically. Once normalized, a query·key dot product is a cosine in $[-1, 1]$: too flat for softmax to be selective, so we scale it back up by a single learnable scalar temperature `s_qk` (initialized to the $\sqrt{d_k}$ that standard attention would use), the mirror image of the usual $1/\sqrt{d_k}$ down-scaling. nGPT proper learns this as a per-channel vector; a lone scalar seems to be enough.

RoPE rotates the vector components in pairs, each pair treated as a 2D vector on a plane. The planes are all distinct, so while the embedding as a whole is a single direction vector, it isn't rotated as a whole. I wonder if doing so would improve things further?
...
The attention mechanism works like a look-up table (LUT), where each token embedding can be used to "look up" contextual information. But unlike a regular LUT, this one is
(a) computed on the fly from the incoming token embedding, and
(b) read across the whole sequence at once, weighting each earlier contribution by its relevance to the current token.
...
It's surprising that the value can be computed up-front, even before the queries have been compared to other tokens' keys. It works because the context a token would provide does not depend on what the query is; only how much that value influences the output depends on how closely the key matches the query.
...
There is one nGPT wrinkle. The input to the MLP is a unit vector, and (after normalization) so are the rows of the up-projection, so their dot products land around $1/\sqrt{d}$ — close enough to zero that GELU would behave like a straight line, losing the non-linearity. To fix this we multiply the up-projection by a $\sqrt{n_\text{embd}}$ baseline (and a single learnable scalar `s_u`), pushing the pre-activations out to roughly unit scale, where GELU's curve bends. nGPT proper uses a SwiGLU here with its own gate scaling; we keep the simpler GELU and restore its operating range.

[^og]: From 1958!

### What it does

Exactly what is going on in the large matrices that project up and then back down is anyone's guess. But essentially, it takes each embedding from the attention block — which by now contains contextual information from earlier embeddings in the sequence — and it _does knowledge_ to it. That is, it looks at the embedding and says something about it that it thinks will be useful for later layers. Both halves of that are learned during training: "something" is another embedding that has meaning, and "will be useful" is the MLP's own judgment.
...
3. Adds the knowledge it has learned to the contextualized tokens with a simple feed-forward network (see _MLP_, above)
...
These blocks are then stacked as "layers"; see _Decoder-only transformer_, below.

### Residual connection on the sphere

In a standard transformer the residual update is a plain addition: `h ← h + attn(h)`. That has a couple of motivations. Numerically, it lets gradients propagate all the way back through the network; without it, earlier layers would struggle to learn anything. It also allows some of the original information to flow past each layer — otherwise the meaning of the token would be completely replaced by whatever the attention mechanism thought was useful, which may starve later layers of the information _they_ need.

nGPT keeps that spirit but reframes it geometrically. It takes a small step from $h$ _towards_ the sub-module's output direction, then re-normalizes back onto the sphere. The target is the sub-module's normalized output $\hat{h}_\text{sub} = \text{Norm}(\text{sub}(h))$, so the update is a spherical interpolation (LERP-then-project) between two unit vectors:
...
Normalizing the sub-module output is load-bearing. The target has to be a unit vector for $\alpha$ to mean "rotate by $\alpha$": the MLP scales its pre-activations by a $\sqrt{n_\text{embd}}$ baseline (see _MLP_, above), so $\lVert\text{MLP}(h)\rVert \propto \sqrt{n_\text{embd}}$ — around 7, 10, 15 at width 32, 64, 128 — and a step toward the _raw_ output would rotate by $\alpha\,\lVert\text{sub}(h)\rVert$, growing with width instead of holding at $\alpha$. The [ngpt-scaling](../../../docs/ngpt-scaling/experiment.py) experiment confirms this holds in training: converged loss is flat across depth at width 128 (~1.33 at 4, 8, and 12 layers) and across the whole width × depth grid.

We strip one thing from the published recipe: the learning. nGPT learns this gate per channel (the paper's _eigen learning rates_), whereas we use a single scalar fixed at $1/n_\text{layer}$ — the value the learned gates settled near anyway, and the sweep suggests a fixed scalar is enough.


## [Decoder-only transformer](ngpt.py)

Here we tie together all the previous modules into "the transformer". The NGPT module:
...
Because the hidden state is a unit vector by the time it reaches the head, a raw logit is again a cosine in $[-1, 1]$ — `s_z` restores the dynamic range that the final softmax needs, as `s_qk` did inside attention. (nGPT makes this a per-class vector; like the other scales we collapse it to one scalar.)

### Keeping the weights on the sphere

Normalizing the _activations_ is only half of nGPT. The weight matrices are constrained too: after every optimizer step we re-normalize each matrix that reads from or writes to the residual stream, so its rows (or columns) are unit vectors. This happens in the training loop rather than the forward pass — `model = model.normalize_weights()` after each optimizer step — and it means the matrices act as rotations on the sphere rather than arbitrary linear maps. A side effect is that weight decay becomes redundant: the norm is already pinned to 1, so there is nothing for decay to shrink.

### Interpretation of the logits

During training, the "labels" $y$ are the same as the input sequence $x$ but shifted by one (so that $y_t=x_{t+1}$). The output logits have a length equal to the vocabulary size[^vocab], so we can interpret them as meaning "the likelihood that token $k$ comes next". That meaning isn't intrinsic to logits; it comes from the loss function, which is not part of the model — it lives in the training code.
