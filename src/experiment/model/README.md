...
Normalizing the sub-module output is load-bearing. The target has to be a **unit** vector for $\alpha$ to mean "rotate by $\alpha$": the MLP deliberately scales its pre-activations by a $\sqrt{n_\text{embd}}$ baseline (see _MLP_, above), so $\lVert\text{MLP}(h)\rVert \propto \sqrt{n_\text{embd}}$ — around 7, 10, 15 at width 32, 64, 128 — and a step toward the _raw_ output would rotate by $\alpha\,\lVert\text{sub}(h)\rVert$, growing with width instead of holding at $\alpha$. Normalizing the target pins the per-layer step to $\alpha$, so the stack's travel stays $O(1)$ regardless of scale. The [ngpt-scaling](../../../docs/ngpt-scaling/experiment.py) experiment confirms this holds in training: converged loss is flat across depth at width 128 (~1.33 at 4, 8, and 12 layers) and across the whole width × depth grid.

What we _do_ strip from the published recipe is the learning: nGPT learns this gate per **channel** (the paper's _eigen learning rates_), whereas we use a single scalar fixed at $1/n_\text{layer}$ — the value the learned gates settled near anyway, and the sweep confirms a fixed scalar is enough.
